...
namespace
{
    Foam::word replicatedParticleCloudName(const Foam::word& cloudName)
    {
        const Foam::string shard(Foam::getEnv("FOAM_PARTICLE_SHARD"));
        const Foam::string nShards(Foam::getEnv("FOAM_N_PARTICLE_SHARDS"));

        if (!shard.empty() && !nShards.empty() && std::atoi(nShards.c_str()) > 1)
        {
            return Foam::word
            (
                cloudName + "_shard" + Foam::name(std::atoi(shard.c_str()))
            );
        }
    ...
    Foam::label replicatedParticleSeed()
    {
        const Foam::string shard(Foam::getEnv("FOAM_PARTICLE_SHARD"));
        const Foam::string nShards(Foam::getEnv("FOAM_N_PARTICLE_SHARDS"));
        ...
        if (!shard.empty() && !nShards.empty() && std::atoi(nShards.c_str()) > 1)
        {
            return Foam::Random::defaultSeed
                 + 104729*(std::atoi(shard.c_str()) + 1);
        }

        return Foam::Pstream::myProcNo();
    }
}
...
:
    CloudType(rho.mesh(), replicatedParticleCloudName(cloudName), false),
    kinematicCloud(),
    ...
    rndGen_(replicatedParticleSeed()),
    cellOccupancyPtr_(),
    ...
    log(true)
{
    const string shard(getEnv("FOAM_PARTICLE_SHARD"));
    const string nShards(getEnv("FOAM_N_PARTICLE_SHARDS"));

    if (!shard.empty() && !nShards.empty() && std::atoi(nShards.c_str()) > 1)
    {
        Info<< "    Particle shard output cloud: " << this->name() << endl;
        Info<< "    Particle shard random seed: " << rndGen_.seed() << endl;
    }
