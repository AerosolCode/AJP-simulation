...
:
    ...
    logFrequency_(1),
    maxCo_(0.3),
    limitTrackTimeFraction_(true),
    iter_(1),
    trackTime_(0.0),
    deltaTMax_(GREAT),
    analyticPositionTracking_(false),
    eventDriven_(false),
    eventTrackTime_(0.0),
    stopWhenCloudEmpty_(true),
    coupled_(false),
...
:
    ...
    maxCo_(cs.maxCo_),
    limitTrackTimeFraction_(cs.limitTrackTimeFraction_),
    iter_(cs.iter_),
    trackTime_(cs.trackTime_),
    deltaTMax_(cs.deltaTMax_),
    analyticPositionTracking_(cs.analyticPositionTracking_),
    eventDriven_(cs.eventDriven_),
    eventTrackTime_(cs.eventTrackTime_),
    stopWhenCloudEmpty_(cs.stopWhenCloudEmpty_),
...
void Foam::cloudSolution::read()
{
    ...
    dict_.readIfPresent("limitTrackTimeFraction", limitTrackTimeFraction_);
    dict_.readIfPresent("deltaTMax", deltaTMax_);
    dict_.readIfPresent("analyticPositionTracking", analyticPositionTracking_);
    dict_.readIfPresent("eventDriven", eventDriven_);
    dict_.readIfPresent("eventTrackTime", eventTrackTime_);
    dict_.readIfPresent("stopWhenCloudEmpty", stopWhenCloudEmpty_);

    if (deltaTMax_ <= 0)
    {
        FatalIOErrorInFunction(dict_)
            << "deltaTMax must be greater than zero" << nl
            << exit(FatalIOError);
    }

    if (eventDriven_)
    {
        if (eventTrackTime_ <= 0)
        {
            dict_.readIfPresent("maxTrackTime", eventTrackTime_);
        }

        if (eventTrackTime_ <= 0)
        {
            FatalIOErrorInFunction(dict_)
                << "eventDriven tracking requires eventTrackTime "
                << "or maxTrackTime greater than zero" << nl
                << exit(FatalIOError);
        }

        Info<< "Cloud event-driven tracking enabled: trackTime = "
            << eventTrackTime_ << " s";

        if (deltaTMax_ < GREAT)
        {
            Info<< ", deltaTMax = " << deltaTMax_ << " s";
        }
